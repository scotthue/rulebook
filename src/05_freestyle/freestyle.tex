...
\textbf{Note:} Age groups and categories may be different for different types of event.
The minimum allowable age groups and categories are listed for each event.
Convention hosts are free to add more age groups and but additional categories can only be added when agreed upon by the Artistic Director and Chief Judge.
Age group is determined by the rider's age on the first day of the convention.
Expert is open to riders of any age.
Riders must state the category in which they are entering for each artistic event in which they participate.

\textbf{Example:} Riders who enter Individual Freestyle as Experts can enter Pairs in Intermediate if they wish.
...
There must be a minimum of five Technical judges and five Presentation judges for Expert Individual and Pairs competitions.
There must be a minimum of three Technical judges and three Presentation judges for all other Freestyle competitions.
More judges may be added but there must be an odd and equal number of judges for both Technical and Presentation. 
...
See section \ref{subsec:freestyle_judging-panel_nominating-freestyle-judges} for description of which teams are required to provide judges.
...
\subsection{Assignment Of Judges}
...
The individual members of the judging panel must remain the same for entire categories; for example one judge may not be replaced by another except between categories.
...
Parties (Clubs) that participate at competitions must nominate judges in relation to the number of Freestyle participants they send (see table below). 
...
Judge nominations include the experience of the judges (such as previous competitions, how long he/she has been judging or other relevant qualifications such as educations).
...
A minimum of 30 minutes must be set aside, in a classroom or similar environment.
...
\section{North American Champions \label{sec:freestyle_na-champions}}
Winners in the Expert category at of each event at NAUCC are the \textbf{North American Champions}.
In the individual events, separate titles are awarded for male and female.

\chapter{Freestyle Rules}

\section{Individual Freestyle Overview \label{sec:freestyle_individual-freestyle-overview}}

\subsection{Categories and Time Limits}
Categories are determined by skill level.
The USA Skill Levels are used as a guide to determine level of skill.
...
\subsubsection{Choosing Categories}
Riders may enter the competition category they wish according to the approximate skill level of the skills planned for the routine.
...
When choosing categories, riders should bear in mind that Freestyle competition is judged 50\% on difficulty (skill level), and 50\% on presentation.

\subsubsection{Promoting Rider(s) to a Higher Category}
If a routine has more than three skills successfully performed that are higher than the allowed level for the category, the judges should make a note of this during the routine and notify the Chief Judge.
Skills successfully performed is defined as performing the skill for a reasonable distances without falling given the choreography of the routine.
Obviously not all skills are listed in the skill levels, so judges will need to use their knowledge of the difficulty of skills to determine approximate skill level.
If the majority of the judges agree with the assessment that there are more than three skills performed that are higher than the allowed level (with the Chief Judge adding a deciding vote if there is an even number of judges), the rider(s) scoring will be removed from their current category and the rider will be asked to perform again in the next higher category.
...
\subsection{Categories of Competition and Time Limits}
For Pairs Freestyle the skill levels of the two riders should be averaged to determine category placement.
All the rest of the rules are the same as Individual Freestyle (\ref{sec:freestyle_individual-freestyle-overview}).
...
Group freestyle is divided into two categories: Club Show and Group. 
Each rider may enter each category (Club Show and Group) only once. 
For example: a rider can be in Club Show and Group Routine, but not two Group Routines.
...
Six minutes for Club Show, three minutes for Group.
...
The suggested minimum size for Freestyle event must be 28m x 15m.
...
Performance order for Freestyle competitions are defined by a random drawing.
